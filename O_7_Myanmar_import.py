...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2020년 9월 1일 ~ 2021년 4월 30일
    SQLDATE BETWEEN 20200901 AND 20210430
    -- 국가: 미얀마(MMR) 또는 버마(BM)
    AND (
      Actor1CountryCode IN ('MMR', 'BM') OR
      Actor2CountryCode IN ('MMR', 'BM') OR
      ActionGeo_CountryCode IN ('MMR', 'BM')
    )
)
-- EventRootCode만 집계
SELECT
...
# =======================================
print("\n[INFO] BigQuery에서 실제 데이터를 불러오는 중입니다 (미얀마)...")
...
# =======================================
# a. 이벤트 날짜 및 윈도우 정의
EVENT_DATE = pd.to_datetime('2021-02-01') # 미얀마 쿠데타 시작일
...
# c. '이상 징후' 기간 (1/2 ~ 1/31)에 해당하는 행의 라벨을 1로 변경
mask = (root_ratio.index >= ANOMALY_START_DATE) & (root_ratio.index < EVENT_DATE)
root_ratio.loc[mask, 'is_anomaly'] = 1

# d. 이벤트 발생 당일(2/1) 및 그 이후 데이터는 학습에서 제외
df_labeled_myanmar = root_ratio[root_ratio.index < EVENT_DATE].copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print(f"\n[SUCCESS] 미얀마 (2021-02-01 기준) 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_myanmar.shape}")

print("\n--- '이상 징후' 시작 지점 데이터 (Label=1) ---")
print(df_labeled_myanmar[df_labeled_myanmar['is_anomaly'] == 1].head(3))

print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_myanmar[df_labeled_myanmar['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_myanmar.to_csv("labeled_myanmar.csv")
print("\n[INFO] 'labeled_myanmar.csv' 파일로 저장 완료.")