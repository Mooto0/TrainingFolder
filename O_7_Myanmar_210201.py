...
print("[INFO] BigQuery에서 집계된 데이터를 불러오는 중입니다 (미얀마)...")
...
# 2021년 2월 1일에 세로 점선 추가 (미얀마 쿠데타)
ax.axvline(pd.to_datetime('2021-02-01'), color='red', linestyle='--', linewidth=1.5, label='Coup d\'état Start (2021-02-01)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily QuadClass Ratio (Myanmar, Sep 2020 - Apr 2021)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
# 2021년 2월 1일에 세로 점선 추가
ax.axvline(pd.to_datetime('2021-02-01'), color='red', linestyle='--', linewidth=1.5, label='Coup d\'état Start (2021-02-01)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily Ratio of All EventRootCodes (Myanmar, Sep 2020 - Apr 2021)")
plt.xlabel("Date")
