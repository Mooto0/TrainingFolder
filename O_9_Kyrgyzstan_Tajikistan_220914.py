...
print("[INFO] BigQuery에서 집계된 데이터를 불러오는 중입니다 (키르기스스탄/타지키스탄)...")
...
# 2022년 9월 14일에 세로 점선 추가 (국경 분쟁 격화)
ax.axvline(pd.to_datetime('2022-09-14'), color='red', linestyle='--', linewidth=1.5, label='Border Conflict (2022-09-14)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily QuadClass Ratio (Kyrgyzstan/Tajikistan, Jun-Dec 2022)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
# 2022년 9월 14일에 세로 점선 추가
ax.axvline(pd.to_datetime('2022-09-14'), color='red', linestyle='--', linewidth=1.5, label='Border Conflict (2022-09-14)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily Ratio of All EventRootCodes (Kyrgyzstan/Tajikistan, Jun-Dec 2022)")
plt.xlabel("Date")
