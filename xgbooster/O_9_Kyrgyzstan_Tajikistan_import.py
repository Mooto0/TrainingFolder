...
# =======================================
# 3. GDELT 데이터 쿼리 (키르기스스탄/타지키스탄, 2022년)
# =======================================
...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2022년 6월 1일 ~ 2022년 12월 31일
    SQLDATE BETWEEN 20220601 AND 20221231
    -- 국가: 키르기스스탄(KGZ) 및 타지키스탄(TJK)
    AND (
      Actor1CountryCode IN ('KGZ', 'TJK') OR
      Actor2CountryCode IN ('KGZ', 'TJK') OR
      ActionGeo_CountryCode IN ('KGZ', 'TJK')
    )
)
-- EventRootCode만 집계
SELECT
...
# =======================================
# 3.5. 쿼리 실행 전 건조 실행(Dry Run) 및 용량 확인
# =======================================
...
# =======================================
# 3.8. 실제 쿼리 실행 (사용자가 'y'를 입력한 경우)
# =======================================
print("\n[INFO] BigQuery에서 실제 데이터를 불러오는 중입니다 (키르기스스탄/타지키스탄)...")
...
# =======================================
# a. 이벤트 날짜 및 윈도우 정의
EVENT_DATE = pd.to_datetime('2022-09-14') # 국경 분쟁 격화일
WINDOW_DAYS = 30  # 기준일 직전 30일을 '이상 징후'로 정의
ANOMALY_START_DATE = EVENT_DATE - pd.Timedelta(days=WINDOW_DAYS)
# ANOMALY_START_DATE = '2022-08-15'

# b. 'is_anomaly' 컬럼을 0으로 초기화
root_ratio['is_anomaly'] = 0

# c. '이상 징후' 기간 (8/15 ~ 9/13)에 해당하는 행의 라벨을 1로 변경
mask = (root_ratio.index >= ANOMALY_START_DATE) & (root_ratio.index < EVENT_DATE)
root_ratio.loc[mask, 'is_anomaly'] = 1

# d. 이벤트 발생 당일(9/14) 및 그 이후 데이터는 학습에서 제외
df_labeled_kyr_taj = root_ratio[root_ratio.index < EVENT_DATE].copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print(f"\n[SUCCESS] 키르기스스탄-타지키스탄 (2022-09-14 기준) 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_kyr_taj.shape}")

print("\n--- '이상 징후' 시작 지점 데이터 (Label=1) ---")
print(df_labeled_kyr_taj[df_labeled_kyr_taj['is_anomaly'] == 1].head(3))

print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_kyr_taj[df_labeled_kyr_taj['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_kyr_taj.to_csv("labeled_kyr_taj.csv")
print("\n[INFO] 'labeled_kyr_taj.csv' 파일로 저장 완료.")