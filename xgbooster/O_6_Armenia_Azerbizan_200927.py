...
print("[INFO] BigQuery에서 집계된 데이터를 불러오는 중입니다 (아르메니아/아제르바이잔)...")
...
# 2020년 9월 27일에 세로 점선 추가 (2차 나고르노-카라바흐 전쟁)
ax.axvline(pd.to_datetime('2020-09-27'), color='red', linestyle='--', linewidth=1.5, label='War Start (2020-09-27)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily QuadClass Ratio (Armenia/Azerbaijan, Jun 2020 - Jan 2021)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
# 2020년 9월 27일에 세로 점선 추가
ax.axvline(pd.to_datetime('2020-09-27'), color='red', linestyle='--', linewidth=1.5, label='War Start (2020-09-27)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily Ratio of All EventRootCodes (Armenia/Azerbaijan, Jun 2020 - Jan 2021)")
plt.xlabel("Date")
