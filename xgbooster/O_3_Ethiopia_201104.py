...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2020년 6월 1일 ~ 2021년 1월 31일로 변경
    SQLDATE BETWEEN 20200801 AND 20201231
    -- 국가: 에티오피아(ET)
    AND (
      Actor1CountryCode = 'ETH' OR
      Actor2CountryCode = 'ETH' OR
      ActionGeo_CountryCode = 'ETH'
    )
)
...
# 2020년 11월 4일에 세로 점선 추가
ax.axvline(pd.to_datetime('2020-11-04'), color='red', linestyle='--', linewidth=1.5, label='Tigray War Start (2020-11-04)')

ax.xaxis.set_major_locator(mdates.MonthLocator(interval=1))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
plt.xticks(rotation=45)
# 그래프 제목 수정
plt.title("Daily QuadClass Ratio (Ethiopia, Jun 2020 - Jan 2021)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
plt.xticks(rotation=45)
# 그래프 제목 수정
plt.title("Daily Ratio of All EventRootCodes (Ethiopia, Jun 2020 - Jan 2021)")
plt.xlabel("Date")
