...
# =======================================
# 3. GDELT 데이터 쿼리 (에티오피아, 2020-2021년)
# =======================================
# '이상 징후' 기간(10-11월)과 그 이전의 '정상' 기간(6-9월)을 모두 포함
AGGREGATED_QUERY = """
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2020년 6월 1일 ~ 2021년 1월 31일 (사용자 제공 그래프 기준)
    SQLDATE BETWEEN 20200601 AND 20210131
    -- 국가: 에티오피아(ETH) (내전이므로 ETH만 필터링)
    AND (
      Actor1CountryCode IN ('ETH') OR
      Actor2CountryCode IN ('ETH') OR
      ActionGeo_CountryCode IN ('ETH')
    )
)
-- EventRootCode만 집계
SELECT
...
print("[INFO] BigQuery에서 데이터를 불러오는 중입니다 (에티오피아)...")
...
# =======================================
# a. 이벤트 날짜 및 윈도우 정의
EVENT_DATE = pd.to_datetime('2020-11-04') # 티그라이 전쟁 시작일
WINDOW_DAYS = 30  # 분쟁 직전 30일을 '이상 징후'로 정의
ANOMALY_START_DATE = EVENT_DATE - pd.Timedelta(days=WINDOW_DAYS)
# ANOMALY_START_DATE = '2020-10-05'

# b. 'is_anomaly' 컬럼을 0으로 초기화
root_ratio['is_anomaly'] = 0

# c. '이상 징후' 기간 (10/5 ~ 11/3)에 해당하는 행의 라벨을 1로 변경
mask = (root_ratio.index >= ANOMALY_START_DATE) & (root_ratio.index < EVENT_DATE)
root_ratio.loc[mask, 'is_anomaly'] = 1

# d. 이벤트 발생 당일(11/4) 및 그 이후 데이터는 학습에서 제외
df_labeled_ethiopia = root_ratio[root_ratio.index < EVENT_DATE].copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print("\n[SUCCESS] 에티오피아 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_ethiopia.shape}")

print("\n--- '이상 징후' 시작 지점 데이터 (Label=1) ---")
print(df_labeled_ethiopia[df_labeled_ethiopia['is_anomaly'] == 1].head(3))

print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_ethiopia[df_labeled_ethiopia['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_ethiopia.to_csv("labeled_ethiopia.csv")
print("\n[INFO] 'labeled_ethiopia.csv' 파일로 저장 완료.")