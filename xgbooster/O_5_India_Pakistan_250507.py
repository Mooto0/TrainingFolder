...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2025년 1월 1일 ~ 2025년 7월 31일로 변경
    SQLDATE BETWEEN 20250101 AND 20250731
    -- 국가: 인도(IN)와 파키스탄(PK)으로 변경
    AND (
      Actor1CountryCode IN ('IN', 'PK') OR
      Actor2CountryCode IN ('IN', 'PK') OR
      ActionGeo_CountryCode IN ('IN', 'PK')
    )
)
...
plt.xticks(rotation=45)
# 그래프 제목 수정
plt.title("Daily Ratio of All EventRootCodes (India/Pakistan, Jan - Jul 2025)")
plt.xlabel("Date")
