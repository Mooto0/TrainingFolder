...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2018년 1월 1일 ~ 2019년 12월 31일
    SQLDATE BETWEEN 20180101 AND 20191231
    -- 국가: 스위스(CHE)
    AND (
      Actor1CountryCode IN ('CHE') OR
      Actor2CountryCode IN ('CHE') OR
      ActionGeo_CountryCode IN ('CHE')
    )
)
-- EventRootCode만 집계
SELECT
...
# =======================================
print("\n[INFO] BigQuery에서 실제 데이터를 불러오는 중입니다 (스위스)...")
...
# =======================================
# 5. 레이블링 (Labeling) - '정상' 데이터
# =======================================
# '정상' 데이터이므로, 모든 행에 대해 'is_anomaly' 값을 0으로 설정합니다.
root_ratio['is_anomaly'] = 0

# '정상' 데이터는 쿼리한 전체 기간을 사용합니다.
df_labeled_switzerland = root_ratio.copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print(f"\n[SUCCESS] 스위스 (정상 데이터) 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_switzerland.shape}")

# '정상' 데이터만 있으므로 Label=1 샘플은 출력하지 않습니다.
print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_switzerland[df_labeled_switzerland['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_switzerland.to_csv("labeled_switzerland.csv")
print("\n[INFO] 'labeled_switzerland.csv' 파일로 저장 완료.")