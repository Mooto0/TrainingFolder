...
# =======================================
print("\n[INFO] BigQuery에서 실제 데이터를 불러오는 중입니다 (니제르)...")
...
# =======================================
# a. 이벤트 날짜 및 윈도우 정의
EVENT_DATE = pd.to_datetime('2023-07-26') # 니제르 쿠데타 발생일
WINDOW_DAYS = 30  # 기준일 직전 30일을 '이상 징후'로 정의
ANOMALY_START_DATE = EVENT_DATE - pd.Timedelta(days=WINDOW_DAYS)
# ANOMALY_START_DATE = '2023-06-26'

# b. 'is_anomaly' 컬럼을 0으로 초기화
root_ratio['is_anomaly'] = 0

# c. '이상 징후' 기간 (6/26 ~ 7/25)에 해당하는 행의 라벨을 1로 변경
mask = (root_ratio.index >= ANOMALY_START_DATE) & (root_ratio.index < EVENT_DATE)
root_ratio.loc[mask, 'is_anomaly'] = 1

# d. 이벤트 발생 당일(7/26) 및 그 이후 데이터는 학습에서 제외
df_labeled_niger = root_ratio[root_ratio.index < EVENT_DATE].copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print(f"\n[SUCCESS] 니제르 (2023-07-26 기준) 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_niger.shape}")

print("\n--- '이상 징후' 시작 지점 데이터 (Label=1) ---")
print(df_labeled_niger[df_labeled_niger['is_anomaly'] == 1].head(3))

print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_niger[df_labeled_niger['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_niger.to_csv("labeled_niger.csv")
print("\n[INFO] 'labeled_niger.csv' 파일로 저장 완료.")