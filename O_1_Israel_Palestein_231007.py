...
# 2023년 10월 7일에 세로 점선 추가
ax.axvline(pd.to_datetime('2023-10-07'), color='red', linestyle='--', linewidth=1.5, label='Hamas Attack (2023-10-07)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily QuadClass Ratio (Israel/Palestine, Jun-Dec 2023)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily Ratio of All EventRootCodes (Israel/Palestine, Jun-Dec 2023)")
plt.xlabel("Date")
