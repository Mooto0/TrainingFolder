...
print("[INFO] BigQuery에서 집계된 데이터를 불러오는 중입니다 (아프가니스탄)...")
...
# 2021년 8월 15일에 세로 점선 추가 (카불 장악)
ax.axvline(pd.to_datetime('2021-08-15'), color='red', linestyle='--', linewidth=1.5, label='Fall of Kabul (2021-08-15)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily QuadClass Ratio (Afghanistan, Jan-Oct 2021)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
# 2021년 8월 15일에 세로 점선 추가
ax.axvline(pd.to_datetime('2021-08-15'), color='red', linestyle='--', linewidth=1.5, label='Fall of Kabul (2021-08-15)')

ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
plt.xticks(rotation=45)
plt.title("Daily Ratio of All EventRootCodes (Afghanistan, Jan-Oct 2021)")
plt.xlabel("Date")
