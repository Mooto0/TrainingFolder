...
WITH RelevantEvents AS (
  ...
  WHERE
    -- 기간: 2022년 12월 1일 ~ 2023년 6월 30일로 변경
    SQLDATE BETWEEN 20221201 AND 20230630
    -- 국가: 수단(SD)으로 변경
    AND (
      Actor1CountryCode = 'SDN' OR
      Actor2CountryCode = 'SDN' OR
      ActionGeo_CountryCode = 'SDN'
    )
)
...
# <<< 2023년 4월 15일에 세로 점선 추가 >>>
ax.axvline(pd.to_datetime('2023-04-15'), color='red', linestyle='--', linewidth=1.5, label='Sudan Conflict Start (2023-04-15)')

ax.xaxis.set_major_locator(mdates.MonthLocator(interval=1)) # 1개월 간격으로 눈금 표시
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
plt.xticks(rotation=45)
# 그래프 제목 수정
plt.title("Daily QuadClass Ratio (Sudan, Dec 2022 - Jun 2023)")
plt.xlabel("Date")
plt.ylabel("Ratio")
plt.legend()
plt.tight_layout()
...
plt.xticks(rotation=45)
# 그래프 제목 수정
plt.title("Daily Ratio of All EventRootCodes (Sudan, Dec 2022 - Jun 2023)")
plt.xlabel("Date")
