...
# =======================================
# 3. GDELT 데이터 쿼리 (뉴질랜드, 2018-2019년)
# =======================================
# '정상' 상태로 간주되는 2년치 기간
AGGREGATED_QUERY = """
WITH RelevantEvents AS (
  ...
  WHERE
    ...
    -- 국가: 뉴질랜드(NZL)
    AND (
      Actor1CountryCode IN ('NZL') OR
      Actor2CountryCode IN ('NZL') OR
      ActionGeo_CountryCode IN ('NZL')
    )
)
-- EventRootCode만 집계
SELECT
...
# =======================================
print("\n[INFO] BigQuery에서 실제 데이터를 불러오는 중입니다 (뉴질랜드)...")
...
df_labeled_newzealand = root_ratio.copy()

# =======================================
# 6. 결과 확인 및 저장
# =======================================
print(f"\n[SUCCESS] 뉴질랜드 (정상 데이터) 레이블링 완료.")
print(f"최종 데이터 Shape: {df_labeled_newzealand.shape}")

# '정상' 데이터만 있으므로 Label=1 샘플은 출력하지 않습니다.
print("\n--- '정상' 기간 데이터 (Label=0) ---")
print(df_labeled_newzealand[df_labeled_newzealand['is_anomaly'] == 0].head(3))

# 이 데이터프레임을 나중에 다른 사례와 합치기 위해 파일로 저장합니다.
df_labeled_newzealand.to_csv("labeled_newzealand.csv")
print("\n[INFO] 'labeled_newzealand.csv' 파일로 저장 완료.")